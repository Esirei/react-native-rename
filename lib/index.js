    ...
    var resolveJavaFiles = function resolveJavaFiles() {
      return new Promise(function (resolve) {
        readFile(_path2.default.join(__dirname, 'android/app/src/main/AndroidManifest.xml')).then(function (data) {
          ...
          ['MainApplication.java', 'MainActivity.java'].forEach(function (value) {
            var move = _shelljs2.default.exec('git mv ' + fullCurrentBundlePath + '/' + value + ' ' + fullNewBundlePath + '/' + value);
            ...
            if (move.code === 0) {
              ...
              // if "outside repository" error occured
              if (_shelljs2.default.mv('-f', fullCurrentBundlePath + '/' + value, fullNewBundlePath + '/' + value).code === 0) {
                console.log(successMsg);
              } else {
